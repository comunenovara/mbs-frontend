...
export class MbsIncentiveCalculationNewUpdateFormComponent extends EngeLibGenericForm {
	...
	@Input() procurement: MbsGovernativeProcurementLotDto | undefined;
	...
	_filteredProcurement: Observable<MbsGovernativeProcurementLotDto[]>;
	...
	override loadForm(): void {
		this._newUpdateForm = this._formBuilder.group({
			id: [null],
			description: [null, [  ]],
			procurement: [this.procurement, [ EngeValidator.haveId,  ]],
			regulation: [this.regulation, [ EngeValidator.haveId,  ]],
		});

		this._filteredProcurement = this.mbsIncentiveAutocompleteService.filterGovernativeProcurementLot(this._newUpdateForm.controls['procurement'].valueChanges);
	...
	override prepareResult(): MbsIncentiveCalculationDto {
		...
		{
			result.procurementId = (result.procurement != null) ? result.procurement.id : undefined;
